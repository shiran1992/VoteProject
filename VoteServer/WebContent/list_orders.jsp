...
<script>
	...
	function del() {
		...
		if (checkcount == 0) {
			...
			oForm.action = "/HomeMaking/DeleteOrderServlet";
			oForm.submit();
		}
	}
					...
					<table class="table">
						...
						<c:forEach var="order" items="${orders}">
							<tr class='even'>
								...
								<td align=center><c:out value="${order.server.sid}"></c:out></td>
								<td align=center><c:out value="${order.create_time}"></c:out></td>
								<td align=center><c:out value="${order.all_price}"></c:out></td>
								<td align=center><c:out value="${order.server_time}"></c:out></td>
								<td align=center><c:out value="${order.state}"></c:out></td>
								<td align=center><c:out value="${order.comment}"></c:out></td>
							</tr>
						</c:forEach>
